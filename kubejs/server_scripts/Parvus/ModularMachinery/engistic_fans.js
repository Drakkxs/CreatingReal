    ...
    function constructRecipe(recipe, XP_PER, XP_MULTI, XP_ID, MIN_TIME, TIME_MULTI, MIN_ENERGY_COST) {
        ...
        return {
            ...
            jsonToItem(strItemStack) {
                ...
                // Flatten basePredicate if present
                if ('basePredicate' in raw) {
                    // Copy all properties from basePredicate into raw
                    Object.assign(raw, raw.basePredicate);
                    delete raw.basePredicate;
                }
             ...
             */
            getItems(keys) {
                // A log of information for debugging. First space acts as a boolean to activate logging.
                let log = [debug, "Log"];
                ...
                if (log[0]) log.push(`Raw Results for ${keys}: ${raw.toString()}`);
                ...
                if (log[0]) log.push(`JSON Elements: ${json.toString()}`);
                ...
                let final = json.map(element => {
                    ...
                    if (log[0]) log.push(`Mapped Item: ${JSON.stringify(objITEM)}`);
                    return objITEM;
                }).reduce((acc, oriItem) => {
                    // Map each ingredient stack to an item object, works for tags.
                    ...
                    if (log[0]) log.push(`Stacks: ${stacks.toString()}`);

                    return acc.concat(stacks);
                }, []);

                if (debug) console.log(`Final Found: ${JSON.stringify(final)}`);
                if (!final.length) throw new Error("No final found with information: " + log.slice(1).join("\n"));
                return final;
