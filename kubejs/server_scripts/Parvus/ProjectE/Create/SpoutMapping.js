...
// requires: create
// @ts-check
// Automatically map Spout recipes to ProjectE conversions.

// Immediately Invoked Function Expression to prevent polluting the global namespace
(() => {
    let debug = false; // Want some debug?
    const filePath = 'kubejs/data/projecte/pe_custom_conversions/generated_spout.json';
    ...
    ServerEvents.recipes(event => {
        ...
        event.forEachRecipe({ type: "create:filling" }, recipe => {
            if (debug) {
                console.log(`Found Create Spout recipe: ${recipe.id}`);
            ...
            // Likley killed by the find, so its not a error.
